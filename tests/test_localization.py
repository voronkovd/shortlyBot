...
class TestTranslations:
    ...
    def test_get_text_russian(self):
        ...
        assert get_text('start_description', 'ru') == "Я бот для скачивания видео из социальных сетей!"
        assert get_text('error_unknown', 'ru') == "❌ Произошла неизвестная ошибка. Попробуй еще раз."
    
    def test_get_text_english(self):
        """Тест получения английского текста"""
        assert get_text('start_welcome', 'en', name='Test') == "Hello, Test! 👋"
        assert get_text('start_description', 'en') == "I'm a bot for downloading videos from social networks!"
        assert get_text('error_unknown', 'en') == "❌ An unknown error occurred. Please try again."
...
class TestUtils:
    ...
    def test_t_function_with_user(self):
        ...
        assert t('error_unknown', user=user) == "❌ Произошла неизвестная ошибка. Попробуй еще раз."
    ...
    def test_t_function_with_language(self):
        ...
        assert t('start_welcome', user=user, language='en', name='Test') == "Hello, Test! 👋"
        assert t('start_welcome', user=user, language='ru', name='Тест') == "Привет, Тест! 👋"
    ...
    def test_t_function_formatting(self):
        ...
        assert t('stats_processing_time', user=user, time=5.5) == "Время обработки: 5.5с"
